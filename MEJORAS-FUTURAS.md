...
- ~~**Log de eventos propio en InfluxDB (`cpd_eventos`).**~~ ✅
  Implementado (23/09/2026) — el colector escribe un evento cada vez que
  un sensor BLE se conecta/desconecta, con mensaje y duración, visible
  como tabla en el dashboard. Ver `CHANGELOG.md` 23/09/2026.
- ~~**Un correo por sensor cuando salta la misma alerta a la vez en
  varios sensores.**~~ 🔍 Investigado a fondo (23/09/2026), sin solución
  disponible en Grafana 13.0.2 con provisioning por fichero — causa raíz
  identificada (una ruta interna de Grafana fuerza el agrupamiento antes
  de que la configuración propia se aplique). Sin impacto funcional real:
  cuando solo un sensor tiene el problema, su email ya llega individual.
  Ver `docs/ALERTAS.md` sección 5.7 para el detalle completo y las 5 vías
  probadas. Revisar si una futura versión de Grafana lo soluciona.
